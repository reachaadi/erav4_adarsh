...
(() => {
    ...
    function renderTable(rows) {
        ...
        <td class="p-2 font-mono">${r.params}</td>
    ...
    explainBtn.onclick = async () => {
        const tableData = [];
        const headers = [...archTbody.previousElementSibling.rows[0].cells].map(c => c.innerText);
        for (const row of archTbody.rows) {
            const rowData = {};
            for (let i = 0; i < row.cells.length - 1; i++) { // ignore last cell with button
                rowData[headers[i]] = row.cells[i].innerText;
            }
        ...
        explanationContent.innerText = "Thinking...";
        ...
        try {
            ...
            explanationContent.innerText = "An error occurred.";
